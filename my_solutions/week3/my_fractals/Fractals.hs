...
pixelToCoordinate x y ((minx, maxy), (maxx, miny)) = ((lenX * fst proportion - maxx), (lenY * snd proportion - maxy))
    where lenX = maxx - minx
          lenY = maxy - miny
          proportion = ((realToFrac x / (fst screen)), (realToFrac y / (snd screen)))
